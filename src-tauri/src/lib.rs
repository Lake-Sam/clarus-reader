...
fn save_api_key(provider: String, key: String) -> Result<(), String> {
    ...
    entry(&provider)?.set_password(trimmed).map_err(|e| e.to_string())
}
...
fn has_api_key(provider: String) -> bool {
    entry(&provider).and_then(|e| e.get_password().map_err(|x| x.to_string())).is_ok()
}
...
async fn complete_ai(request: AiRequest) -> Result<String, String> {
    ...
    let key = if provider == "ollama" { String::new() } else {
    ...
    let response = match provider {
        "openai" => {
            ...
            input.extend(request.messages.iter().map(|m| json!({"role":m.role,"content":m.content})));
        ...
        "anthropic" => {
            ...
            client.post(url)
                ...
                .json(&json!({"model":request.model,"system":request.system,"messages":request.messages,"max_tokens":1800}))
                .send().await
        }
        "compatible" => {
    ...
    let value: Value = response.json().await.map_err(|e| format!("Invalid provider response: {e}"))?;
    if !status.is_success() {
        let detail = value.pointer("/error/message").and_then(Value::as_str)
            .or_else(|| value.get("error").and_then(Value::as_str)).unwrap_or("Unknown provider error");
        return Err(format!("Provider returned {status}: {detail}"));
    }
    let text = match provider {
        "openai" => text_from_openai(&value),
        "anthropic" => value.pointer("/content/0/text").and_then(Value::as_str).map(str::to_string),
        "ollama" => value.pointer("/message/content").and_then(Value::as_str).map(str::to_string),
        _ => value.pointer("/choices/0/message/content").and_then(Value::as_str).map(str::to_string),
    };
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![save_api_key, delete_api_key, has_api_key, complete_ai])
